...
- **整日留出** `HOLDOUT_DAYS`：该比赛日的**所有分节**都不进训练集。
  - `2026.08.29`（单节）：**候选测试日**——从没检测/标注过，也不在任何一版自监督
    语料里。规则：**只跑一次、只报告，不参与调参**（看过它就不再无偏，一旦拿它调参
    就降级成验证日）。
  - `2026.08.31`（3 节）：**调参验证日**——已标注、零成本；但它参与过
    keep_thr 0.72→0.75→0.68 的历史，只算"见过的数据"。
...
- 顺序很重要：**先在 08.31 上把要调的调完，再跑 08.29 报告一次**，否则 08.29 的
  结果会被用来做决策，它就不再是测试日了。
...
HOLDOUT_DAYS = (
    "2026.08.29",   # 单节；候选测试日（从没被看过：只跑一次报告，不参与调参）
    "2026.08.31",   # 3 节；调参验证日（参与过 keep_thr 0.72→0.75→0.68 的历史）
)
