...
HOLDOUT_DAYS = ()
# 单场留出（按文件名）。当前保留 08.25-bba半场 这一场：机位是中线/边线交点斜正对篮筐，
# 与全部训练数据（底线机位）不同域，唯一无偏的跨机位泛化样本 —— 实测该场集成 AUC 0.967
# 但自动 √ 召回只有 12.5%（B 臂真球均分 0.205，场内 0.95 AUC 的主力臂在此域崩掉）。
# 除非用户明确决定扩域，否则不要把它放进训练集。
