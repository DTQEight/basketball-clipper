...
- **当前策略：全量投入训练，不留实拍验证日**（用户决定）。手上所有已标注的比赛日
  （08.29 / 08.31 / 09.01 / 09.03 / 09.04 / 09.08 / 09.11 / 09.15）一律进训练集，
  换取约 +30% 的样本量。
...
# ⚠️ 当前为空：用户决定把手上所有已标注的比赛日都投入训练，测试日改用**未来新比赛**。
#    拿到新测试日后务必把它的日期前缀加进这里（例如 "2026.09.2x"），
#    否则它会随训练集一起被吃掉、再也当不了测试日。
HOLDOUT_DAYS = ()
# 历史上的单场留出（09.01-3rd/-4th、09.03-3rd 曾在此），按当前策略同样放开
HOLDOUT_FILES = ()
