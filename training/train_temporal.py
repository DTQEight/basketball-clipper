...
def main():
    ...
    if args.arch == "bigru" and use_aug:
        ...
        # 读-改-写：只更新本脚本负责的字段，**保留其它键**。整表覆盖会丢掉
        # ensemble 段（服务端 goal_verifier 只读它拿权重/阈值），而 ensemble 一旦
        # 缺失就回落到代码默认值（0.70 / b0.5+vm1.0），与标定值（0.68 / b1.0+vm0.5）
        # 不同 —— 等于跑一次训练就把线上三带分诊静默换成了未标定的配置。
        try:
            _old = (json.loads(META_OUT.read_text(encoding="utf-8"))
                    if META_OUT.exists() else {})
        except Exception as e:
            print(f"⚠ 旧 meta 读取失败（按空处理）: {e}")
            _old = {}
        meta = {
            **_old,
        ...
        # 原子写：中断不留半截 meta（半截 meta 会让服务端读到不完整配置）
        _tmp = META_OUT.parent / (META_OUT.name + ".tmp")
        _tmp.write_text(json.dumps(meta, ensure_ascii=False, indent=2),
                        encoding="utf-8")
        _tmp.replace(META_OUT)
