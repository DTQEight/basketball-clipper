...
- **跳帧推理**：默认每 2 帧推理一次（覆盖 50%），可开提速模式每 3 帧推理一次（覆盖 33%）；非推理帧复用 `_last_ball_pos` 填充历史，保持时间窗口完整
- **设备自动检测**：`get_device()` 自动检测 CUDA 可用性，有 GPU 用 `cuda:0`，无 GPU 回退 `cpu`；主线程启动时 `WARMUP-OK` 预热模型与 CUDA 上下文，避免首次推理时驱动崩溃
