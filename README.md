...
最新版本 **2026.09.21**：条件跳过判据改为与斑块门同构（消灭「斑块触发却跳过 YOLO」的静默漏球）、裁剪推理（只推筐接受框 + `imgsz` 1280 → 640，单场实测耗时 −34%）。
上一版 **2026.09.19**：漏检根因修复（YOLO 接受框外扩）、RTX 50 系显卡支持（cu121 → cu128）、Windows 安装程序（Inno Setup）、FRAME 线程解码（端到端提速 1.36×、结果零变化）。
