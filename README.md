...
| 46.286s | ❌ | ❌ | ❌ | 遗留项：warmup 阈值 12 vs 15（`auto_thr` +8 vs +11） |
...
- YOLO 拒绝 491→429→408 单调下降，剩余差距全部指向遗留项，无新增异常
- 剩余 2 漏检与上轮完全相同（46.286 / 556.099），**本轮修复零回归、零副作用**
- **v3 是三代中质量最好的版本**。合并 main 前最后决策点不变：46.286s 人工确认为真实进球 → 修遗留项（+8→+11）；556.099s 建议一并回放
