...
- FFmpeg（含 libx264，由 `imageio-ffmpeg` 自带）
...
```
basketball-clipper/
├── demo_interactive.py     # 交互式 Demo（主入口）
├── tracker.py              # GoalDetector：diff + YOLO 双确认进球检测
├── video_io.py             # PyAV 视频读取（HEVC/moov 兼容）
├── transcoder.py           # GPU 硬解硬编转码
├── audio_detector.py       # 音频峰值检测（可选，融合模式用）
├── app.py                  # 旧版完整界面（3 Tab）
├── demo_chonyy.py          # 旧版 chonyy 状态机 demo
├── cutter/ffmpeg_cutter.py # ffmpeg 剪辑（集锦保持原画质）
├── config.yaml             # 配置文件
├── requirements.txt        # 依赖
...
| 最小进球间隔 | 2 秒 | 两个进球间的最小时间间隔，防止重复计数 |
| 帧差阈值 | 25 | diff 二值化阈值，低=灵敏（降到 15）/ 高=严格（提到 35）|
| 最小斑块面积 | 50 | 过滤小噪声 |
| 最大斑块面积 | 5000 | 过滤大物体（人）|
| 搜索范围 | 60px | 篮筐周边搜索运动物体的范围 |
