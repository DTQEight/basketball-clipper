...
| 12 | 历史记录 + 片段缓存 | detection_history.json 持久化；重启复用预览 |
| 13 | 文件夹批量 + 流水线确认 | 逐个标定后批量识别，视频完成即可预览/导出 |
| 14 | NiceGUI 可视化界面 | 深色主题卡片式；检测中可随时取消；三级调试日志 |
...
**单视频流程**：输入视频路径 → 加载 → 帧选择器选帧 + 2 点标定篮筐 → 设置参数 → 开始识别 → 卡片 √/× 标记 → 导出集锦

**批量流程**：输入文件夹 → 逐个标定（保存标定）→ 批量识别 → 每个视频完成即可流水线查看/标记/导出
...
├── services/
│   ├── detection.py        # 检测调度 / 进度回调 / 三级日志 / 标记与集锦
│   ├── state.py            # detection_history.json（含 labels 训练池）
...
最新版本 **2026.09.03-04（ad 分支）**：√/× 人工标记 + 标签飞轮训练池直通 + 帧选择器 Catch-up + HEVC NAL 容错解码 + 文档结构重组。
