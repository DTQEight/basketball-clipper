...
最新版本 **2026.09.22**：修复三处影响可用性的问题——**手机 HDR 源（10-bit HLG）的预览片段在浏览器里播不了**（NVENC 回退软编时 libx264 按输入位深编出 `h264 High 10`，Chrome 的 H.264 解码链只吃 8-bit → 黑屏且无任何报错；编码参数改为钉死 8-bit SDR 并用 `setparams` 把颜色标记落到 bt709）、**加载 `E:\ball\*.mov` 永久卡死**（PyAV 帧线程死锁，`read_frame` 关闭帧级多线程解码；同时修掉 `start.bat` 用 PowerShell 管道转发 stdout 导致控制台不消费时整条事件循环冻住）；另新增历史记录留档 **AI 当次分带 `verify_snapshot`**（人工确认会把 `labels.auto_*` 搬进 `kept`/`deleted`，模型当次判了什么此前不可回溯、判对率趋势无从统计）。
